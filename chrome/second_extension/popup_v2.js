...
function renderHierarchy(parentId, level, targetElement) {
    ...
    children.forEach(item => {
        ...
        if (item.type === 'folder') {
            // No longer double click to navigate, just select. Navigation happens via expand/collapse
            // itemDiv.addEventListener('dblclick', () => navigateIntoFolder(item.id)); // Keeping for future if needed
        }
...
async function renderFileList() {
    ...
    // Add general dragover/drop for the main fileListDiv acting as currentPath
...
function handleDragOver(e) {
    ...
    const targetElement = e.target.closest('.folder-item'); // Only folder items are explicit drop targets

    if (targetElement && draggedItemId) {
        ...
        e.dataTransfer.dropEffect = 'none'; // No valid target
    }
}
...
// Handler for dropping an item onto the general fileListDiv area (implies dropping into currentPath)
function handleDragOverGeneral(e) {
    e.preventDefault();
    if (draggedItemId) {
        const itemToMove = files[draggedItemId];
        // Only allow drop if it's not the current parent of the dragged item
        if (itemToMove && itemToMove.parent !== currentPath) {
            // Also prevent moving a folder into its own subfolder if currentPath is a descendant
            if (itemToMove.type === 'folder' && isDescendant(currentPath, draggedItemId)) {
                e.dataTransfer.dropEffect = 'none';
...
async function handleDropGeneral(e) {
    ...
    const itemToMove = files[draggedItemId];
    if (itemToMove.parent === currentPath) {
    ...
    // Prevent moving a folder into its own subfolder (if currentPath is a descendant)
    if (itemToMove.type === 'folder' && isDescendant(currentPath, draggedItemId)) {
    ...
    const success = await performMove(draggedItemId, currentPath);
    if (success) {
        hideItemDetails();
        renderFileList();
    }
