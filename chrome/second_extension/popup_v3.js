...
function renderHierarchy(parentId, level, targetElement) {
    ...
    children.forEach(item => {
        ...
        // Removed double click to navigate, as tree view handles it
        // if (item.type === 'folder') {
...
function handleDragOverItem(e) {
    ...
    const targetElement = e.target.closest('.folder-item'); // Ensure target is a folder item
    ...
    if (targetElement && draggedItemId) {
        ...
        e.dataTransfer.dropEffect = 'none'; // Not a valid drop target (e.g., trying to drop on a file)
    }
}
...
function handleDragOverGeneral(e) {
    ...
    if (draggedItemId) {
        ...
        if (itemToMove && itemToMove.parent !== '/' && !(itemToMove.type === 'folder' && isDescendant('/', draggedItemId))) {
