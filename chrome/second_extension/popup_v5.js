...
function handleDragStart(e) {
    ...
    console.log('Drag started for:', files[draggedItemId]?.name, 'ID:', draggedItemId); // Added nullish coalescing
}
...
function handleDragOverItem(e) {
    ...
    // console.log(`Drag Over Item: Valid drop on ${files[targetFolderId]?.name}, dropEffect = move`);
}
...
function handleDragOverGeneral(e) {
    ...
    const itemToMove = files[draggedItemId];
    const targetRootId = '/'; // Dropping onto the general area means dropping into root
    ...
    if (itemToMove.parent !== targetRootId && !(itemToMove.type === 'folder' && isDescendant(targetRootId, draggedItemId))) {
        ...
        // console.log(`Drag Over General: Valid drop on root, dropEffect = move (Item: ${itemToMove.name})`);
        return;
    }
    e.dataTransfer.dropEffect = 'none';
    fileListDiv.classList.remove('drop-target');
