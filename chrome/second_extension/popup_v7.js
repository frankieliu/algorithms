...
const detailTypeSpan = document('detail-type'); // Corrected typo here
...
const cancelRenameBtn = document('cancel-rename-btn'); // Corrected typo here
...
function handleDragOverItem(e) {
    e.preventDefault();
    // *** CRITICAL CHANGE: Stop propagation here to prevent parent from interfering ***
    e.stopPropagation();

    const targetElement = e.target.closest('.folder-item');
    if (!targetElement) {
        e.dataTransfer.dropEffect = 'none';
        // console.log('Drag Over Item: Not over a folder-item. dropEffect = none'); // Keep for debugging if needed
        return;
    }
    ...
    if (!draggedItemId || !files[draggedItemId]) {
        ...
        // console.log('Drag Over Item: No valid dragged item. dropEffect = none'); // Keep for debugging if needed
        return;
    }
    ...
    if (targetFolderId === draggedItemId) {
        e.dataTransfer.dropEffect = 'none';
        targetElement.classList.remove('drop-target');
        // console.log('Drag Over Item: Dropping onto self. dropEffect = none'); // Keep for debugging if needed
        return;
    }
    ...
    if (itemToMove.parent === targetFolderId) {
         e.dataTransfer.dropEffect = 'none';
         targetElement.classList.remove('drop-target');
         // console.log('Drag Over Item: Dropping onto current parent. dropEffect = none'); // Keep for debugging if needed
         return;
    }
    ...
    if (itemToMove.type === 'folder' && isDescendant(targetFolderId, draggedItemId)) {
        e.dataTransfer.dropEffect = 'none';
        targetElement.classList.remove('drop-target');
        // console.log('Drag Over Item: Moving folder into its own subfolder. dropEffect = none'); // Keep for debugging if needed
        return;
    }

    e.dataTransfer.dropEffect = 'move';
    targetElement.classList.add('drop-target');
    // console.log(`Drag Over Item: Valid drop target: ${files[targetFolderId]?.name}. dropEffect = move`); // Keep for debugging if needed
}
...
async function handleDropItem(e) {
    e.preventDefault();
    // *** CRITICAL CHANGE: Stop propagation here for the drop event too ***
    e.stopPropagation();
...
// NOTE: handleDragOverGeneral and handleDropGeneral do *not* need stopPropagation
// because they are only meant to catch drops on the general fileListDiv background
// when no specific child folder catches the event.
