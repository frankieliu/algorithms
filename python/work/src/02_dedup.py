...
def main(previous, current, output_file, combined_file, debug):
    ...
    merged_df = df_new.merge(
        ...
        on="job number",
        indicator=True)
    merged_new_df = merged_df[merged_df['_merge']
                              == 'left_only'].drop('_merge', axis=1)
    if debug:
        print(merged_new_df)
        return
    print(
        f"Before: {len(df_new)}, After: {len(merged_new_df)}, Dups: {len(df_new) - len(merged_new_df)}")
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Dedups any duplication from old in new")
    parser.add_argument("-p", "--previous",
                        default=f"{data_dir}/{date_old}_02_job_number_cum.pkl")
    parser.add_argument("-c", "--current",
                        default=f"{data_dir}/{date_new}_01_job_number.pkl")
    parser.add_argument(
        "-o", "--output", default=f"{data_dir}/{date_new}_02_job_number_dedup.pkl")
    parser.add_argument(
        "-m", "--merged", default=f"{data_dir}/{date_new}_02_job_number_cum.pkl")
